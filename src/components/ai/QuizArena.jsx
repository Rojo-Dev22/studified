  ...
  return (
    <motion.button
      ...
      className={`w-full flex items-start gap-3 p-4 rounded-xl border text-left transition-all duration-200
        ...
        ${!reveal && !checked ? 'border-emerald-500/25 bg-emerald-950/40 hover:border-emerald-400/40 hover:bg-emerald-900/30' : ''}
        ${disabled && !reveal ? 'opacity-90' : ''}
      `}
    >
      <div
        className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 transition-colors
          ${checked && !reveal ? 'border-amber-400 bg-amber-400 text-emerald-950' : 'border-emerald-400/50 bg-emerald-950/80'}
      ...
      <span className="text-sm text-emerald-50/95 leading-relaxed flex-1">{option.text}</span>
...
export default function QuizArena({ quiz, onClose, topic, onGetStudyAdvice }) {
  ...
  const getGradeMessage = (correct, total) => {
    ...
    if (pct >= 0.93) return { emoji: 'ðŸ†', title: `Outstanding (${correct}/15)!`, msg: 'You have mastered this topic! You clearly understand the concepts deeply.' };
    if (pct >= 0.80) return { emoji: 'ðŸŒŸ', title: `Excellent (${correct}/15)!`, msg: 'Strong understanding! A few small areas to polish, but you are well on your way.' };
    if (pct >= 0.60) return { emoji: 'ðŸ“š', title: `Good Effort (${correct}/15)`, msg: 'You have a solid foundation but there are some key areas to review. Focus on the questions you missed.' };
    if (pct >= 0.40) return { emoji: 'ðŸ’ª', title: `Keep Going (${correct}/15)`, msg: 'You are building understanding but need more review. Don\'t be discouraged: this shows exactly what to study.' };
    return { emoji: 'ðŸŽ¯', title: `Starting Point (${correct}/15)`, msg: 'This topic needs more attention. Use the advice below to focus your studies. Every expert was once a beginner!' };
  };

  if (!total) {
    return (
      <div className="rounded-2xl border border-emerald-500/30 bg-emerald-950/50 p-6 text-center text-emerald-200/80 text-sm">
  ...
  return (
    <div className="quiz-arena relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-950 via-teal-950 to-slate-950 shadow-2xl shadow-emerald-900/40">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-24 -right-24 w-64 h-64 rounded-full bg-amber-400/10 blur-3xl" />
        <div className="absolute -bottom-16 -left-16 w-48 h-48 rounded-full bg-emerald-500/15 blur-3xl" />
        <div
      ...
      <div className="relative p-5 md:p-6">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            ...
            <h3 className="text-base font-semibold text-emerald-50 mt-0.5">{quiz.title}</h3>
          </div>
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="text-xs text-emerald-300/70 hover:text-emerald-100 px-2 py-1 rounded-md hover:bg-emerald-800/40"
            >
              Back to chat
            </button>
          )}
        ...
        <div className="h-2 rounded-full bg-emerald-900/80 overflow-hidden mb-1">
          <motion.div
        ...
        <p className="text-[10px] text-emerald-400/80 mb-5">
          {finished ? 'Results unlocked' : `Question ${index + 1} of ${total}`}
          {!finished && !allAnswered && ` Â· ${Object.keys(answers).filter((k) => answers[k]?.length).length}/${total} answered`}
        </p>
        ...
        <AnimatePresence mode="wait">
          {finished ? (
            ...
            >
              {/* Score & Grade Section */}
              <div className="text-center py-6 rounded-xl bg-emerald-900/40 border border-amber-400/20">
                <Trophy className="w-10 h-10 text-amber-400 mx-auto mb-2" />
                <p className="text-sm text-emerald-200/60 mb-1">Your Score</p>
              ...
              {/* Grade Message */}
              <div className="rounded-xl bg-emerald-900/30 border border-emerald-500/20 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-200/80 mt-1">{getGradeMessage(score.correct, score.total).msg}</p>
                  </div>
                </div>
              </div>
                ...
                {questions.map((q, qi) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-emerald-300/70 ml-6">
                        Your answer:{' '}
              ...
              {studyAdvice && (
                <div className="rounded-xl bg-gradient-to-r from-amber-500/10 to-emerald-500/10 border border-amber-400/20 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <Lightbulb className="w-5 h-5 text-amber-400" />
                    <p className="text-sm font-semibold text-amber-300">Personalized Study Advice</p>
                  </div>
                  <div className="prose prose-invert prose-xs max-w-none text-xs text-emerald-200/90 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
              ...
              {loadingAdvice && (
                <div className="text-center py-3">
                  <p className="text-xs text-emerald-400 animate-pulse">Analyzing your results for personalized advice...</p>
                </div>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <Button
                  ...
                  variant="outline"
                  onClick={handleRetry}
                  className="flex-1 border-emerald-500/40 bg-emerald-900/30 text-emerald-100 hover:bg-emerald-800/50 h-10"
                >
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-medium text-emerald-50 leading-snug flex-1">{current.text}</p>
              </div>
              ...
              <div className="flex items-center justify-between gap-2">
                <Button
                  ...
                  className="text-emerald-300 hover:text-emerald-100 hover:bg-emerald-800/40 h-9"
                >
                  ...
                  {questions.map((_, i) => (
                    ...
                    >
                      {(answers[questions[i].id] || []).length > 0 ? (
                        ...
                        <Circle
                          className={`w-3 h-3 ${i === index ? 'text-amber-400' : 'text-emerald-600'}`}
                        />
                      )}
                    </button>
                  ))}
                ...
                {index < total - 1 ? (
                  <Button
                    ...
                    className="bg-amber-400 text-emerald-950 hover:bg-amber-300 h-9 font-semibold"
                  >
                    Next
                  ...
                  <Button
                    ...
                    className="bg-gradient-to-r from-amber-400 to-amber-300 text-emerald-950 hover:opacity-90 h-9 font-semibold disabled:opacity-40"
                  >
                    Finish quiz
                  </Button>
                )}
              ...
              {!allAnswered && index === total - 1 && (
                <p className="text-center text-[10px] text-emerald-400/70 mt-3">
